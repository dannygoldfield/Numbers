...
| `AwaitingSettlement` | Demo 1 local settlement outcome `settled` accepted through `POST /demo/settlement` | `Finalized` | `SettlementRecord`, `FinalizationRecord`, required deferred `InscriptionIntentRecord` | Destination = winning destination. The three required records must be persisted in one atomic canonical commit group. `InscriptionIntentRecord` does not alter auction lifecycle state. No chain confirmation or deadline precondition is required for this Demo 1 local control transition. |
| `AwaitingSettlement` | Demo 1 local settlement outcome `expired` accepted through `POST /demo/settlement` | `Finalized` | `SettlementRecord`, `FinalizationRecord`, required deferred `InscriptionIntentRecord` | Destination = `NullSteward`. The three required records must be persisted in one atomic canonical commit group. `InscriptionIntentRecord` does not alter auction lifecycle state. No chain confirmation or deadline precondition is required for this Demo 1 local control transition. |
| `AwaitingSettlement` | Chain-confirmed settlement before deadline | `Finalized` | `SettlementRecord`, `FinalizationRecord` | Destination = winning destination. Reserved for an implementation slice that enables chain-confirmed settlement. |
| `AwaitingSettlement` | Settlement deadline expired under chain-confirmed settlement semantics | `Finalized` | `SettlementRecord`, `FinalizationRecord` | Destination = `NullSteward`. Reserved for an implementation slice that enables chain-confirmed settlement. |
