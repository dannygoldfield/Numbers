...
| `AwaitingSettlement` | Demo 1 local settlement outcome `settled` accepted through `POST /demo/settlement` | `Finalized` | `SettlementRecord`, `FinalizationRecord` | Destination = winning destination. No chain confirmation or deadline precondition is required for this Demo 1 local control transition. |
| `AwaitingSettlement` | Demo 1 local settlement outcome `expired` accepted through `POST /demo/settlement` | `Finalized` | `SettlementRecord`, `FinalizationRecord` | Destination = `NullSteward`. No chain confirmation or deadline precondition is required for this Demo 1 local control transition. |
